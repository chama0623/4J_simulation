...
int main(int argc,char *argv[]){
    ...
    解析解$\frac{1}{2} \log_{e} 3 = 0.5493061$に十分近い値であることがわかる.解析解に収束する様子や誤差の考察については次節で行う.
    ...
    考えられる.このため,打ち切りが生じる.\\
    　打ち切り誤差は分割数を大きくすれば減少するのであれば,可能な限り分割数を大きくすれば,より正確な数値解を得られる
    ...
    　丸め誤差はコンピュータの扱える桁数に依存する部分があるため,double型の扱える桁数を確認しておく.
    IEEE754によればdouble型(6バイト)の内部表現は符号部1ビット,指数部11ビット,仮数部52ビットであり,
    これは10進数において約15桁の精度である.\\
